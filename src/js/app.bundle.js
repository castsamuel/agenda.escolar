...
function emptyData() {
  return {
    ...
    diary: [],            // registros pessoais {id, date, title, description, consequenceType, attachments:[{id,name,type,size}], createdAt}
    settings: {
      minGrade: 6.0,
...
function addDiaryEntry(fields) {
  const d = load();
  const entry = { id: uid(), createdAt: new Date().toISOString(), attachments: [], consequenceType: 'Nenhuma', ...fields };
  d.diary.push(entry);
  save();
  return entry;
}
function updateDiaryEntry(id, fields) {
  const d = load();
  const e = d.diary.find((x) => x.id === id);
  if (e) { Object.assign(e, fields); save(); }
}
...
// ---------- Controle de notificações de prazo (evita aviso duplicado) ----------
// Guardamos, no próprio item (dever/trabalho), a data do prazo para a qual
// já avisamos (`notifiedForDate`). Se o prazo mudar, essa marca fica
// desatualizada e um novo aviso volta a ser permitido — é exatamente a
// checagem que impede notificação repetida sem precisar de outra tabela.
function markNotified(kind, id, dueDate) {
  const d = load();
  const list = kind === 'homework' ? d.homework : d.works;
  const item = list.find((x) => x.id === id);
  if (item) { item.notifiedForDate = dueDate; save(); }
}
...
return { saveFile, getFile, deleteFile, getAllIds };
...
return { requestPermission, checkUpcomingDeadlines, startWatcher, canNotify };
...
const Page_Diary = (function() {
const db = DB;
const filesdb = FilesDB;
const { openModal } = Modal;
const { toast, confirmDialog, escapeHTML, fmtDate, todayISO } = Utils;
...
const db = DB;
const filesdb = FilesDB;
const { requestPermission, checkUpcomingDeadlines, startWatcher, canNotify } = Notifications;
...
const ROUTES = {
  ...
  '#/diario': { title: 'Diário', mod: Diary, icon: '📓' },
