...
router.post('/setup', async (req, res) => {
  ...
  if (db.prepare('SELECT id FROM users LIMIT 1').get()) return res.status(409).json({ error: 'A conta inicial jÃ¡ foi configurada.' });
  if (!username?.trim() || password?.length < 6 || password !== confirmation) return res.status(400).json({ error: 'Informe um usuÃ¡rio e uma senha de pelo menos 6 caracteres. As senhas devem coincidir.' });
...
router.post('/login', async (req, res) => {
  ...
  if (!user || !(await bcrypt.compare(req.body.password || '', user.password_hash))) return res.status(401).json({ error: 'UsuÃ¡rio ou senha invÃ¡lidos.' });
